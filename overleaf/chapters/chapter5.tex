...
This chapter gets into the details of how KANDA actually works, piece by piece. We explain what each software module does, how they talk to each other, and what happens in a typical control cycle from sensor to motor. We also show what happens when things go wrong. This chapter is where the rubber meets the road---this is what we actually implemented and why each piece is necessary.

%-------------------------------------------------------------------
\section{High-Level Design Overview}
\label{sec:hld_overview}

The deployment comprises a Raspberry Pi running the Python package tree \texttt{vision\_module/}, an ESP32 running Arduino-compatible firmware, a duplex UART link at 115,200 baud, and HTTPS access to two cloud services: Groq (text reasoning and ASR) and NVIDIA NIM (visual understanding). Figure~\ref{fig:hld_overview} summarises the principal deployable blocks.

%--- IMAGE PLACEHOLDER: Replace TikZ diagram with deployment_architecture.png if available
% To use image instead of TikZ:
% \begin{figure}[H]
% \centering
% \includegraphics[width=0.9\textwidth]{deployment_architecture.png}
...
\node[block, fill=cyan!10, right=1.8cm of op] (pi) {\textbf{Raspberry Pi}\\{\footnotesize \texttt{vision\_module/} (Python)}};

\node[block, fill=orange!12, right=1.8cm of pi] (esp) {\textbf{ESP32}\\{\footnotesize firmware executor}};

\node[block, fill=purple!10, below=1.45cm of pi] (gem) {\textbf{Groq + NVIDIA NIM}\\{\footnotesize Text \& Vision APIs}};
...
\draw[arr] (pi.south) -- node[lbl, left] {reasoning} (gem.north);
\draw[arr] (gem.north) -- node[lbl, right] {response} (pi.south);
...
Fig.~\ref{fig:hld_overview} shows how the operator interacts with the intelligence layer on the Pi, how the Pi exchanges newline-terminated JSON and telemetry with the ESP32 over UART, how the ESP32 connects to the physical stack, and how the Pi dispatches to Groq (for text reasoning) and NVIDIA NIM (for visual understanding) over HTTPS.
...
\node[comp, fill=purple!14, right=0.2cm of net] (gem) {\textbf{Groq + NVIDIA NIM}\\{\scriptsize reasoning \& vision APIs}};
...
The Raspberry Pi component hosts the entire \texttt{vision\_module/} tree. The ESP32 component hosts sensing, actuation, OLED rendering, and UART framing. Groq and NVIDIA NIM are external cloud services accessed from the Pi.
...
\node[mod, fill=green!10, below right=0.95cm and 0.15cm of main] (llm) {task\_agent.py\\\scriptsize reasoning + vision};
...
Figure~\ref{fig:state_machine_ch5} shows the state-transition graph for the Phase~4 orchestrator.

\begin{figure}[H]
\centering
\resizebox{0.95\textwidth}{!}{%
\begin{tikzpicture}[
  state/.style={draw, thick, rounded corners=6pt, minimum width=2.2cm,
...
\node[state, fill=green!15, right=of idle]  (listen) {LISTENING};
\node[state, fill=yellow!15, right=of listen] (think)  {THINKING};
\node[state, fill=orange!15, below=of idle]  (act)    {ACTING};
\node[state, fill=red!10, below=of listen]  (search) {SEARCHING};
\node[state, fill=blue!12, below=of think]  (speak)  {SPEAKING};
\node[state, fill=purple!10, below right=1.2cm and 0.4cm of search] (report) {REPORTING};

\draw[trans] (idle)   -- node[above]{wake word / Telegram} (listen);
\draw[trans] (listen) -- node[above]{VAD end} (think);
\draw[trans] (think)  -- node[left,pos=0.3]{COMMAND} (act);
\draw[trans] (think.south) -- node[right,pos=0.3]{TASK / find} (search.north);
\draw[trans] (think) -- node[above right,pos=0.6]{QUESTION} (speak);
\draw[trans] (act)    -- node[left]{done} (idle);
\draw[trans] (search) -- node[below]{found / max steps} (report);
\draw[trans] (speak)  -- node[right]{TTS done} (report);
\draw[trans] (report) -- node[below left]{reset} (idle);
\draw[trans, dashed, red!60!black] (search.west) -- node[above left,font=\scriptsize\itshape]{cancel} (idle.south);
\draw[trans, dashed, red!60!black] (act.west) to[out=180,in=210] node[left,font=\scriptsize\itshape]{obstacle} (idle.west);
\end{tikzpicture}}
\caption{7-state event-driven machine for the Phase~4 multimodal agent. Dashed red transitions represent global cancel (obstacle detection or user stop command).}
\label{fig:state_machine_ch5}
\end{figure}

Four concurrent daemon threads support the state machine: (1)~openWakeWord detector running offline ONNX inference at 8\% CPU, (2)~ESP32 heartbeat monitor parsing telemetry and propagating obstacle events, (3)~Telegram bot long-polling, and (4)~TTS worker queue draining speech requests.

%-------------------------------------------------------------------
\subsection{ReAct Search Algorithm}
\label{subsec:react_search}

For ``find'' tasks, a specialised ReAct loop (Fig.~\ref{fig:react_flow}) combines vision, movement, and semantic memory.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  block/.style={draw, thick, rounded corners=3pt, minimum height=0.8cm,
                minimum width=2.8cm, align=center, font=\footnotesize},
  decision/.style={draw, thick, diamond, aspect=2, inner sep=1pt,
                   align=center, font=\footnotesize},
  arr/.style={-{Stealth[length=2mm]}, thick},
  node distance=0.55cm
]
\node[block, fill=blue!8] (cap) {Capture frame};
\node[block, fill=blue!8, below=of cap] (desc) {VLM: describe scene};
\node[decision, below=of desc] (mem) {In memory?\\(sim $>$ 0.7)};
\node[decision, below left=0.8cm and 0.2cm of mem] (goal) {Target\\found?};
\node[block, fill=green!10, below=0.6cm of goal] (found) {RETURN found};
\node[block, fill=orange!10, below right=0.8cm and 0.2cm of mem] (move) {Best move\\from sensors};
\node[block, fill=orange!10, below=of move] (exec) {Execute + store};

\draw[arr] (cap) -- (desc);
\draw[arr] (desc) -- (mem);
\draw[arr] (mem) -- node[left,font=\tiny]{no} (goal);
\draw[arr] (mem) -- node[above,font=\tiny]{yes (skip)} (move);
\draw[arr] (goal) -- node[left,font=\tiny]{yes} (found);
\draw[arr] (goal.east) -- node[above,font=\tiny]{no} (move.west);
\draw[arr] (move) -- (exec);
\draw[arr] (exec.east) -- ++(0.8,0) |- node[right,font=\tiny,pos=0.25]{next step} (cap.east);
\end{tikzpicture}
\caption{ReAct search flowchart: capture $\to$ describe $\to$ memory check $\to$ goal check $\to$ move $\to$ loop. Semantic memory (similarity threshold 0.7, auto-lowered to 0.5 after 5 skips) prevents revisiting.}
