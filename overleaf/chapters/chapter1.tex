...
This chapter introduces \textbf{Knowledge-driven Autonomous Navigation and Decision-making Agent (KANDA)}, a multimodal embodied robot agent that we built to explore how Large Language Models can control a physical robot safely and reliably on a budget. We started this project with a simple question: how cheaply could we build a robot that listens, sees, reasons, and moves without crashing? What we discovered along the way taught us more about AI safety, hardware constraints, and embodied intelligence than we expected. This chapter covers the context, motivation, technical challenges we faced, what we built to solve them, and how the rest of the report is organized.
...
Recent advances in Large Language Models (LLMs) have produced strong capabilities in natural language understanding, contextual reasoning, and structured output generation~\cite{vaswani2017attention}. These capabilities enable a different approach to robot control: rather than encoding every possible scenario in conditional logic, a robot can describe its sensory state to an LLM and receive a contextually appropriate action command in return. This approach, termed \textit{embodied AI}, bridges the gap between language-based reasoning and physical action execution~\cite{driess2023palme}.

KANDA addresses this opportunity by implementing a complete multimodal embodied robot agent in which an ESP32 microcontroller serves as the physical executor (body) and a Raspberry Pi~4 running the \texttt{vision\_module/} intelligence layer serves as the cognitive reasoner (brain). The system operates as a closed-loop sense--reason--validate--act pipeline: ultrasonic sensors and (in Phase~4) a Pi Camera measure the environment; sensor telemetry and optional scene descriptions are assembled into a body-context prompt dispatched to Groq (text reasoning) or NVIDIA NIM (vision); the returned JSON command is validated through a safety layer; and the validated command drives the motors. If cloud inference fails or an obstacle is detected, the system gracefully degrades to firmware-level emergency stops or AUTO\_MODE, ensuring continuous safe operation. Figure~\ref{fig:hw_block} (Chapter~\ref{ch:srs}) shows the hardware interconnection; an assembled prototype photograph may be added to \path|images/kanda_robot.jpg| when available for the viva binder.
...
From an engineering perspective, the project integrates embedded systems programming (ESP32/Arduino), hardware interfacing (ultrasonic sensors, motor drivers, voltage dividers), inter-processor communication (UART/serial), cloud API integration (Gemini), and software safety engineering into a single cohesive system, making it a complete study in applied software and hardware co-design.
...
\textbf{AI reasoning:}
\begin{itemize}[nosep]
  \item \textbf{Groq} (\texttt{llama-3.3-70b-versatile}) for text reasoning and ASR; \textbf{NVIDIA NIM} (\texttt{meta/llama-3.2-11b-vision-instruct}) for vision; keys via \texttt{GROQ\_API\_KEY} and \texttt{NVIDIA\_API\_KEY}
...
The project follows a phased iterative development methodology (Fig.~\ref{fig:phases}):
...
\end{tikzpicture}
\caption{Phased development roadmap of KANDA.}
\label{fig:phases}
\end{figure}
